...
RDS master password and Redis AUTH are **required inputs** (`db_password`, `redis_auth_token`). They are **not** generated by Terraform. They still enter remote state as provider attributes on the RDS / ElastiCache resources (unavoidable for those APIs). Mitigations already mandatory for this stack:
...
- Seed Secrets Manager from the operator/CI secret store — **not** from Terraform seed outputs
...
# certificate_arn, domain_name, db_password, redis_auth_token,
# tf_state_bucket, tf_lock_table (HTTPS defaults on)
```
...
Terraform only creates secret **shells**. After apply, construct URLs in your shell from **operator-held passwords** + hostname outputs (OIDC deploy role or local AWS CLI). Do **not** rely on removed `seed_*` outputs.

Percent-encode passwords/tokens before embedding them in URLs (Redis AUTH may contain `#`, `&`, etc. — same as Terraform `urlencode`).
...
DB_PASSWORD='YOUR_RDS_PASSWORD'          # same value passed as db_password
...
| `TF_DB_PASSWORD`        | RDS master password (required for apply)                                |
...
- Plan job uses **only** `AWS_ROLE_ARN_PLAN` (PR + `main` subjects, read-only)
