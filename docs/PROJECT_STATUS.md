...
- Test footprint rộng (verified 2026-07-28): **1375/1375 unit api pass** (snapshot 2026-07-14 với 974/974 là dữ liệu lịch sử) **+ 103/103 unit game-core + 148/148 unit web + 11/11 E2E pass**. Track D replay-delta tests (`packages/game-core/src/match-state-machine.spec.ts` +33, `apps/api/src/gateways/handlers/match.handler.spec.ts` Plan D +202, `apps/web/src/stores/socket-store.updaters.spec.ts` Group H +117) đã cộng dồn vào anchor này. Coverage tách biệt theo package: `@arena/api` aggregate ~98.9% statements (`game-loop.service.ts` 99.6%, `admin.service.ts` 100%, `match.service.ts` 100%); `@arena/game-core` 93.28% overall / 99.22% `src/` statements (`match-state-machine.ts` 99.75%).
