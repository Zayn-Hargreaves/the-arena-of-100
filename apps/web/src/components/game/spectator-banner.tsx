...
/**
 * Thin top-of-page strip telling a drop-in spectator or an eliminated player
 * they are in spectator mode.
 */
export const SpectatorBanner: React.FC<SpectatorBannerProps> = ({
  isEliminated = false,
}) => {
  ...
  return (
    ...
    >
      <MiniGlyph
        variant="eye"
        className="w-5 h-5 text-candy-blue stroke-[2.5] shrink-0 mt-0.5"
      />
      <div className="space-y-0.5">
        <h3 className="font-display font-black text-sm text-candy-ink uppercase tracking-wider">
          {isEliminated ? t("spectatorMode.title") : tSpectator("bannerTitle")}
        </h3>
