...
export const LeaveRoomModal: FC<LeaveRoomModalProps> = ({
  open,
  onOpenChange,
  onConfirm,
  isHost = false,
}) => {
  const t = useTranslations("lobby.leaveModal");
  ...
  return (
    <Modal
      ...
      title={t("title")}
      description={isHost ? t("hostDescription") : t("memberDescription")}
      closeLabel={t("close")}
    >
      <div className="flex flex-col gap-4 mt-4">
        <div className="flex items-start gap-3 p-3 rounded-xl bg-candy-cloud border-[3px] border-candy-ink">
          ...
          <p className="text-sm font-semibold leading-relaxed text-candy-ink">
            {isHost ? t("hostWarning") : t("memberWarning")}
          </p>
        </div>
